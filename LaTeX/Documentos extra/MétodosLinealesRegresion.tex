...
\noindent Otra forma de afrontar el ajuste del modelo desde el punto de vista del algebra lineal. 

\begin{defi}
Llamamos $C_{p+1}(\mathbf{X})$ al subespacio lineal de $\mathbb{R}^{N}$ generado por las columnas de la matriz $\mathbf{X}$
\end{defi}
\begin{propo}
...
\noindent Para poder hacer los estadígrafos de contrastes se debe construir un estimador de esta varianza, ya habitualmente no es conocida. Para ello \emph{Hastie et. al.} proponen el siguiente estimador de la varianza, suponiendo que unicamente hay una variable respuesta: 
\begin{equation}
