...
\begin{equation}
L(\theta)=\sum_{i=1}^n \mathbb{P}_{\theta}(y_i)
\end{equation}

\noindent Suponiendo que es cierto el modelo aditivo se tiene que $Y=f_\theta (\textbf{x})+\varepsilon$, donde $\varepsilon \thicksim \mathcal(0,\sigma^2)$, provoca que si se suponen conocidos a priori el parámetro $\theta$ y el vector aleatorio $\textbf{x}$ entonces :
\begin{equation}
...
Esta reducción de características a considerar permite hacer mucho más interpretable el modelo obtenido durante todo el proceso de ajuste y en algunos casos incluso aumentar la precisión del modelo ya que puede ocurrir que estemos eliminando variables que introduzcan ruido en el modelo. 
...
\begin{equation}
RSS(\beta)=\sum_{i=1}^n(\textbf{y}_i-\textbf{x}_i^T\beta)^2+\lambda\sum_{j=1}^p\beta_j^2
\end{equation}
