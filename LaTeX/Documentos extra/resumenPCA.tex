...
\end{align*}
\noindent d
onde $\mu$ es el vector de medias del vector aleatorio $\mathbf{x}$. 
...
\end{equation}
donde $\overline{x}_j$ es la media muestral de la $j$-ésima variable. Esto hace que $\textbf{S}=\frac{1}{n}\textbf{X}^T\textbf{X}$ \cite{Peña 2002}. Esto permite hablar indistintamente de los vectores propios de $\mathbf{S}$ y $\mathbf{X}^T\mathbf{X}$ ya que estarán cambiados por un escalar.
