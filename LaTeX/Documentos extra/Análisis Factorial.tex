...
\noindent Una vez estudiado esto se puede ver que el modelo no es único \cite{Chatfield 1989}
\begin{propo}
La matriz $\mathbf{\Lambda}$ y $\mathbf{\Lambda T}$ donde $\mathbf{T}$ es ortogonal, son ambas matrices válidas para el modelo. 
\begin{proof}
En el 
\end{proof}
\end{propo}
\noindent Es por ello, que se utilizan ciertas restricciones dependiendo del método utilizado para realizar las estimaciones y que el modelo sea único. 
